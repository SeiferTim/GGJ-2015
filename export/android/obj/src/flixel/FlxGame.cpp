...
Void FlxGame_obj::onEnterFrame( Dynamic _){
{
		...
		if (((bool(!(this->_lostFocus)) || bool(!(::flixel::FlxG_obj::autoPause))))){
			...
			this->draw();
			HX_STACK_LINE(550)
			this->debugger->stats->visibleObjects(::flixel::FlxBasic_obj::visibleCount);
			HX_STACK_LINE(551)
...
Void FlxGame_obj::step( ){
{
		...
		this->update();
		HX_STACK_LINE(693)
		this->debugger->stats->activeObjects(::flixel::FlxBasic_obj::activeCount);
	}
return null();
}
...
Void FlxGame_obj::draw( ){
{
		...
		HX_STACK_LINE(867)
		this->debugger->stats->drawCalls(::flixel::graphics::tile::FlxTilesheet_obj::_DRAWCALLS);
		HX_STACK_LINE(893)
		{
			HX_STACK_LINE(893)
			int _g = (int)0;		HX_STACK_VAR(_g,"_g");
